...
public class Enemy : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        GameObject NearestPlayer = GameManager.Instance.NearestPlayer(CurrPos);

        if(NearestPlayer != null){
            Vector3 PlayerPos = NearestPlayer.transform.position;
            Vector3 dirVect = PlayerPos - CurrPos;
            Vector3 normalizedVect = dirVect.normalized;
            transform.rotation = Quaternion.LookRotation(normalizedVect, Vector3.forward);
            transform.rotation *= Quaternion.Euler(0,90,0);
            mRigidBody.MovePosition(transform.position + normalizedVect * speed * Time.deltaTime);
        }
