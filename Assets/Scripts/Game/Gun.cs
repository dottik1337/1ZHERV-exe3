...
public class Gun : MonoBehaviour
{
    ...
    public void ShootGun(Transform director)
    {
        ...
        if(shotgun){
            float angleBetweenBullets = shotgunSpread / (float)shotgunBullets;
            Vector3 playerDir = director.rotation.eulerAngles;

            for(int i = 0; i<shotgunBullets; i++){
                SpawnBullet(
                director.position, 
                Quaternion.Euler(0.0f,0.0f, playerDir.z - shotgunSpread / 2.0f + i * angleBetweenBullets)
        );
            }
        }
        else{
